...
export function AudioPlayer({ title, speaker, duration, audioUrl }: AudioPlayerProps) {
  ...
  return (
    <div className="bg-bg2 border border-border-subtle rounded-2xl p-6 animate-fade-up">
      {audioUrl && (
        <audio
          ref={audioRef}
          src={audioUrl}
      ...
      <div className="flex items-start justify-between mb-5">
        ...
        <div className="bg-scribia-red/8 border border-scribia-red/20 rounded-lg px-3 py-1.5 text-[11px] text-[#FF7A90] flex items-center gap-1.5">
      ...
      {/* Waveform */}
      <div className="flex items-center gap-[2px] h-12 mb-4 cursor-pointer" onClick={seek}>
        {barHeights.map((h, i) => (
          <div
            key={i}
      ...
      <div className="flex items-center gap-4">
        <span className="text-xs text-text3 tabular-nums min-w-9">
        ...
        <div className="flex items-center gap-2.5">
          <button
            onClick={() => skip(-10)}
            className="w-9 h-9 rounded-full flex items-center justify-center text-text2 hover:text-text transition-colors"
          >
            <SkipBack className="w-4 h-4" />
          </button>
          <button
            onClick={togglePlay}
            className="w-11 h-11 rounded-full bg-purple flex items-center justify-center text-white glow-purple hover:bg-purple-light transition-all"
          >
            {isPlaying ? (
